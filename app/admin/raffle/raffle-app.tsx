...
import { CheckCircle2, Dices, Gift, LoaderCircle, Maximize2, Minimize2, Sparkles, Trophy, X } from "lucide-react";
...
export function AdminRaffleScreen() {
  ...
  const [reveal, setReveal] = useState<RaffleResult | null>(null);
  ...
  const spin = async () => {
    ...
    if (!window.confirm("Run the official raffle now? This selects one winner, saves it permanently, and publishes it to the home page.")) return;
    setMessage("");
    setReveal(null);
    setSpinning(true);
    try {
      ...
      setPendingResult(officialResult);
  ...
  return (
    <AdminFrame active="raffle">
      ...
      {loading ? (
        ...
        <section className={`${isExpanded ? "fixed inset-0 z-[120] min-h-dvh overflow-y-auto bg-[var(--surface)] p-5 sm:p-8 xl:grid-cols-[minmax(0,1fr)_360px] xl:items-center" : "relative rounded-[24px] border-hairline border-line bg-white p-4 shadow-[0_18px_48px_rgba(var(--brand-deep-rgb),0.08)] sm:p-6 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-center"} grid gap-5`} aria-label={isExpanded ? "Expanded raffle draw" : "Raffle draw"}>
          <button className={`${isExpanded ? "fixed right-5 top-5 border-white/20 bg-brand-deep text-white shadow-[0_12px_30px_rgba(0,0,0,0.2)]" : "absolute right-4 top-4 border-line bg-white text-brand shadow-[0_8px_20px_rgba(var(--brand-deep-rgb),0.1)]"} tap-card z-30 inline-flex !w-auto shrink-0 items-center justify-center gap-2 whitespace-nowrap rounded-full border px-4 py-3 text-center text-[12px] font-semibold`} type="button" onClick={() => setIsExpanded((current) => !current)} aria-label={isExpanded ? "Exit full-screen raffle" : "Expand raffle to full screen"}>
            {isExpanded ? <Minimize2 size={17} /> : <Maximize2 size={17} />}
            {isExpanded ? "Exit full screen" : "Expand raffle"}
          </button>
          <div className="grid justify-items-center gap-5">
            <div className="raffle-wheel-stage relative aspect-square w-full max-w-[720px]" style={isExpanded ? { maxWidth: "min(76vh, 820px)" } : undefined}>
            ...
            <button className="tap-card inline-flex min-h-14 w-full max-w-[430px] items-center justify-center gap-2 rounded-full bg-brand-deep px-7 text-[17px] font-bold text-white shadow-[0_16px_34px_rgba(var(--brand-deep-rgb),0.25)] transition hover:bg-brand-mid disabled:cursor-not-allowed disabled:opacity-55" type="button" onClick={spin} disabled={!payload.ready || Boolean(payload.result) || spinning}>
              {spinning ? <><LoaderCircle className="animate-spin" size={20} /> Drawing the official winner…</> : payload.result ? <><CheckCircle2 size={20} /> Official winner selected</> : <><Sparkles size={20} /> Run official draw</>}
            </button>
          </div>
...
function RaffleWinnerReveal({ result, onClose }: { result: RaffleResult; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-[140] grid place-items-center overflow-hidden bg-brand-deep/94 p-5 backdrop-blur-xl" role="dialog" aria-modal="true" aria-labelledby="raffle-winner-name">
      ...
      <button className="tap-card absolute right-5 top-5 z-20 grid h-11 !w-11 place-items-center rounded-full border border-white/20 bg-white/10 text-center text-white" type="button" onClick={onClose} aria-label="Close raffle winner celebration"><X size={20} /></button>
